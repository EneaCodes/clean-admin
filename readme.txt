...
* Supports translations via `bros-clean-admin-hide-dashboard-ads` text domain
...
1. Upload the `bros-clean-admin-hide-dashboard-ads` folder to the `/wp-content/plugins/` directory, or install via the Plugins → Add New screen.
2. Activate **Bros Clean Admin – Hide Dashboard Ads** through the "Plugins" menu in WordPress.
...
* `brosclad_promo_words` – filter the default promo words array.
* `brosclad_review_words` – filter the default review words array.
...
= 1.1.1 =
WordPress.org compliance update: Fixed function prefixes, proper CSS enqueuing, and contributor information. All settings will be preserved during update.
...
1. Clean Admin settings page - choose which ads/nags to hide and add custom keywords
